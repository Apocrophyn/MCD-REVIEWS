...
export function QueueScreen({ receipts, historyOnly, busy, onOpen, onRetry, onCancel, onArchive }: Props) {
  ...
  return <div className="page queue-page">
    <header className="queue-header"><div><h1>{historyOnly ? "History" : "Your queue"}</h1><p>{historyOnly ? "Completed surveys." : "Pick up exactly where you left off."}</p></div><span>{receipts.length} total</span></header>
    ...
    {!receipts.length || (historyOnly && !completed.length) ? <div className="queue-empty"><Inbox /><h2>Nothing here yet</h2><p>{historyOnly ? "Completed surveys will appear here." : "Upload a receipt to start your queue."}</p></div> : null}
  </div>;
}
...
function QueueGroup({ title, icon, receipts, tone, busy, onOpen, onRetry, onCancel, onArchive }: GroupProps) {
  ...
  return <section className={`queue-group tone-${tone}`}><div className="queue-group-title">{icon}<h2>{title}</h2><span>{receipts.length}</span></div><div className="queue-list">
    {receipts.map((receipt) => <article className="queue-row" key={receipt.id}>
      <button className="queue-main" onClick={() => onOpen(receipt)}><span className={`queue-status status-${receipt.status}`}>{statusIcon(receipt.status)}</span><span><strong>{receipt.store || "Receipt awaiting review"}</strong><small>{receipt.scheduledAt ? `Reminder ${formatWhen(receipt.scheduledAt)}` : formatWhen(receipt.updatedAt)} · {statusLabel(receipt.status)}</small>{receipt.failureReason ? <em>{receipt.failureReason}</em> : null}</span><ChevronRight /></button>
